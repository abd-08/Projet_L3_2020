...
class PDF_WriteTag extends FPDF
{
    ...
    function titre_paragraphe ( $txt){
        $this->SetTextColor(0,0,0);
        $this->SetFont("times", "BU",13);
        $this->Cell(0,0,$txt ,0,1,'L',false);
    }
	...
	function SetStyle($tag, $family, $style, $size, $color, $indent=-1 , $highlight=false)
	{
		 ...
		 $this->TagStyle[$tag]['highlight']=$highlight;
	}
	...
	function TitreChapide($numero){

        $this->SetFillColor(40,40,255);
        $this->SetFont($this->TagStyle["p"]['family'], "BU",14);
        $this->Cell($this->wLine+5,$this->hLine+5,"Texte :".$numero ,0,0,'L',true);
	...
	function DoStyle($tag) // Applies a style
	{
		...
		if ($this->TagStyle[$tag]['highlight']) $this->highlight=true;
	...
	function FindStyle($tag, $ind) // Inheritance from parent elements
	{
		...
		$this->TagStyle[$ind]['highlight']=$this->TagStyle[$tag]['highlight'];
	}
	...
	function PrintLine()
	{
		...
		for($i=0; $i<count($this->Line2Print); $i++)
		{
			...
			if(isset($this->TagHref[$i]))
				...
				$href='';
			if($this->Line2Print[$i]=="")
				$this->Cell($space,$this->hLine,"         ",0,0,'C',$this->highlight,$href);
			else{
                $this->Cell($this->StringLength[$i],$this->hLine,$this->Line2Print[$i],0,0,'C',$this->highlight,$href);
            }
